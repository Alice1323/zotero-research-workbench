...
test("research panel runtime wires selected paper PDF attachment status", () => {
  ...
  assert.match(runtime, /function readSelectedPaperPdfAttachment/);
...
test("research panel runtime wires provider request guards", () => {
  ...
  assert.match(runtime, /function createLlmRuntimeGuard/);
  assert.match(runtime, /function assertLlmRuntimeRequestAllowed/);
  assert.match(runtime, /function estimatePromptTokens/);
  assert.match(runtime, /WorkbenchLlmRuntimeGuard/);
  assert.match(runtime, /输入内容超过单任务 Token 上限/);
  assert.match(runtime, /请求过于频繁，请稍后再试/);
});
...
test("research panel runtime wires local export and import actions", () => {
  ...
  assert.match(runtime, /verifyZipExportFile/);
  assert.match(runtime, /ZIP 导出包为空或缺少 manifest\.json/);
  assert.match(runtime, /ZIP 导出包为空或缺少 snapshot\.json/);
  assert.match(runtime, /PR_RDWR/);
  assert.match(runtime, /@mozilla\.org\/io\/string-input-stream;1/);
  assert.match(runtime, /setUTF8Data/);
  assert.doesNotMatch(runtime, /convertToInputStream/);
  assert.match(runtime, /tryZoteroFilePicker/);
  assert.match(runtime, /pickComponentsFile/);
  assert.match(runtime, /showFilePicker/);
  assert.match(runtime, /initFilePicker/);
  assert.match(runtime, /filePickerParentCandidates/);
  assert.match(runtime, /useBrowsingContext/);
  assert.match(runtime, /parentWindow\?\.browsingContext/);
  assert.match(runtime, /exportWorkbenchStateToDefaultFile/);
  assert.match(runtime, /pickDefaultWorkbenchExportFile/);
  assert.match(runtime, /getDesktopDirectory/);
  assert.match(runtime, /保存对话框不可用，已导出到/);
  assert.match(runtime, /null\s*\]/);
  assert.match(runtime, /picker\.open\(resolve\)/);
...
test("research panel runtime wires WebDAV export target actions", () => {
  ...
  assert.match(runtime, /function requestWebDav/);
